cd dist
rem start java -cp RVision.jar rvision.UdpCameraServer -s COM1 -p 4000
rem start java -cp RVision.jar rvision.HttpCameraServer 4000 4002
...
REM "%VLC%" dshow:// :dshow-vdev="%VDEV%" :dshow-adev="none" :dshow-size="%SIZE%" --sout #transcode{vcodec=mp4v,vb=%BITRATE%,scale=1,fps=10}:duplicate{dst=std{access=http,mux=ts,dst=:%HTTP_PORT%},dst=rtp{dst=127.0.0.1,port=6974,sdp=file://c:\Movies\camera.sdp}}
